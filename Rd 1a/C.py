...
def prep():
    bads = frozenset(bad_shuffle() for _ in xrange(100000))
    pickle.dump(bads, open('bads.p', 'w'))

def solve(perm, bads): return 'BAD' if tuple(perm) in bads else 'GOOD'

def main(fin, fout=None):
    T = int(fin.readline().strip())
    potential_bad = pickle.load(open('bads.p', 'r'))
    for t in xrange(T):
...
if __name__ == '__main__':
    ...
    if mode is 'practice':
        ...
        with open(problem+'-small-attempt3.in', 'r') as fin:
